...
class RtpVideoPlayer(
    ...
    private fun drainDecode() {
        ...
        try {
            ...
            while (true) {
                ...
                c.queueInputBuffer(inIndex, 0, au.data.size, queuedNs / 1000, 0)
    ...
    private fun recoverDecoderStallIfNeeded(frames: Int, accessUnits: Int) {
        ...
        if (accessUnits <= 0 || !codecConfigured || !hasDecodedFrames()) {
            return
        }
        ...
        if (BuildConfig.DEBUG && ClientFileLog.logConnections) {
            ClientFileLog.conn(
                "video decoder stall reset port=$listenPort au_delta=$accessUnits " +
                    "zero_frame_heartbeats=$streak",
            )
        }
    }
