...
Artwork is local data, not session protocol payload. Hosts and clients should use the same local asset provider against a configurable assets root. With the default ROM root `<Gaming>/ROMS/Games`, assets default to `<Gaming>/ROMS/Assets`.
