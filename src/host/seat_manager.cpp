...
void SeatManager::set_host_player_count(std::uint8_t player_count) {
    if (player_count > 1) {
...
std::uint8_t SeatManager::host_player_count() const {
    return host_player_count_;
}

SeatAssignment SeatManager::assign(const std::vector<ClientSeatRequest>& remote_requests) const {
