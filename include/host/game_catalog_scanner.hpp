...
inline std::string display_name_from_path(const std::filesystem::path& content_path) {
    return content_path.stem().string();
}

inline std::string lower_string(std::string value) {
...
inline bool path_contains_component(const std::filesystem::path& path, std::initializer_list<std::string_view> names) {
...
inline bool extension_in(std::string_view extension, std::initializer_list<std::string_view> allowed) {
    return std::find(allowed.begin(), allowed.end(), extension) != allowed.end();
}

inline std::optional<std::string> infer_system_key_from_path(const std::filesystem::path& content_path) {
...
inline void replace_all(std::string& value, std::string_view from, std::string_view to) {
...
inline std::string fold_common_latin_accents(std::string value) {
...
inline std::string canonical_token(std::string value) {
...
inline std::string identity_key_for(
...
inline std::string asset_key_for(
    std::string_view system_key,
    std::string_view canonical_name,
    std::string_view language,
    std::string_view region,
...
inline std::filesystem::path default_metadata_root_for(const std::filesystem::path& content_root) {
    return content_root.parent_path() / "Meta";
}

inline std::filesystem::path metadata_path_for(
...
inline std::uint64_t file_update_time(const std::filesystem::path& path) {
...
inline std::uint64_t game_update_time(
    const std::filesystem::path& content_path,
...
inline void apply_game_metadata(GameInfo& info, const std::filesystem::path& metadata_path) {
...
inline void finalize_game_identity(GameInfo& info) {
...
inline GameCatalog scan_game_catalog(
    const std::filesystem::path& content_root,
